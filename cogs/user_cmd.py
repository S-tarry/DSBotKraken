...
from config.config import NOT_REGIST_ID, REGIST_ID, PRIORITY_COLORS
...
class CmdUsers(commands.Cog):
    ...
    async def mytasks(self, inter: disnake.ApplicationCommandInteraction):
        ...
        for tasks in tasks_info:
            embed = tasks_info_embed(tasks.id, tasks.title, tasks.description, tasks.status, tasks.task_priority, tasks.role, tasks.price, tasks.xp)
